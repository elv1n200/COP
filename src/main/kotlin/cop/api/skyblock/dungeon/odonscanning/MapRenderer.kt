...
import cop.api.abobaui.dsl.onClick
import cop.api.abobaui.dsl.onMouseEnterExit
...
import cop.api.abobaui.dsl.seconds
...
import cop.utils.render.DrawContextUtils.drawImage
...
object MapRenderer {
    ...
    fun ElementScope<*>.renderMap(config: MapConfig): ElementScope<*> {
        ...
        return block(
            ...
            colour = colour { DungeonMap.bgColour.rgb },
            (radius * 1.5f).radius()
        ) {
            // Border is always declared; alpha drops to 0 when disabled so toggling is live.
            outline(
                colour = colour {
                    if (DungeonMap.border) DungeonMap.borderColour.rgb
                    else Colour.TRANSPARENT.rgb
                },
                thickness = DungeonMap.borderThickness.px
            )

            // Rebuild the inner tree if any structural setting (sizes, toggles) changes.
            // Colours are already reactive via `colour { ... }` and don't need rebuilds.
            watch({
                listOf<Any>(
                    DungeonMap.showHeads,
                    DungeonMap.showOwnHead,
                    DungeonMap.iconScale,
                    DungeonMap.iconBorder,
                    DungeonMap.classColour,
                    DungeonMap.iconBorderThickness,
                    DungeonMap.showNames,
                    DungeonMap.whenLeap,
                    DungeonMap.nameScale,
            ...
            val refreshable = refreshableGroup(constrain(Centre, Centre, Copying - thickness, Copying - thickness)) {
                ...
                group(constrain(Centre, Centre, dynamicW.px, dynamicH.px)) {
                    ...
                    if (config.icons) renderIcons(new, dynamicW, dynamicH)
                }
            }
    ...
    private fun ElementScope<*>.renderIcons(config: MapConfig, width: Float, height: Float) {
        ...
        Dungeon.dungeonTeammates.forEach { player ->
            ...
            // Position the column so the head's visual centre (head + border)
            // sits on the player's map position. The element's drawing area is
            ...
            column(constrain(x, z, Bounding, Bounding)) {
                object : Element(size(w.px, h.px)) {
                    ...
                    override fun drawCtx() {
                        ...
                        withScale {
                            ctx.pose().translate(wi / 2f, hi / 2f)
                            ctx.pose().rotate((180.0 + player.yaw()).rad)
                            ctx.pose().translate(-wi / 2f, -hi / 2f)
                            when {
                                self && !useHead -> ctx.drawImage(GREEN_MARKER, 0, 0, wi, hi)
                                useHead -> player.playerSkin?.let {
                                    // Read picker fresh each frame so live colour changes apply.
                                    val col = if (classColour) player.clazz.colour else DungeonMap.iconBorderColour

                                    if (border) {
                                        // Ring around the head: drawn first, head on top.
                                        ctx.rect(-t / 2f, -t / 2f, (wi + t).toInt(), (hi + t).toInt(), col.rgb)
                                    }
                                    PlayerFaceRenderer.draw(ctx, it, 0, 0, wi)
                                } ?: ctx.drawImage(WHITE_MARKER, 0, 0, wi, hi)
                                else -> ctx.drawImage(WHITE_MARKER, 0, 0, wi, hi)
                            }
                        }
                    }
                }.add()
